...
local function _bl()
    ...
    card.Size=UDim2.new(0,310,0,120)
    ...
    _T:Create(card,TweenInfo.new(0.35,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),
        {Position=UDim2.new(0.5,-155,1,-135)}):Play()
    ...
    return ss,sb2,se,sv,cl
end

task.spawn(function()
    local ss,sb,se,sv,cl=_bl()
    ...
    local dt
    pcall(function() dt=_HS:JSONDecode(rp) end)
    ...
    if not dt.ok then
        ...
        if dt.reason==string.char(101,120,112,105,114,101,100) then
            ...
            sb(string.char(72,87,73,68,58,32)..hw,Color3.fromRGB(220,160,80))
        end
        cl(5);return
    end
