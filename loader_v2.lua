...
local RADAR_RADIUS = 74
local RADAR_RANGE  = 250
local RADAR_POS_X  = 10
local RADAR_POS_Y  = 10
...
local function onBigHeadToggle(s)
	MISC.BigHead = s
	if s then startBigHeadLoop() else stopBigHeadLoop(); restoreHeads() end
end
			...
			for _,obj in ipairs(workspace:GetDescendants()) do
				if obj:IsA("BasePart") and not wallBangCache[obj] then
					...
					if not isCharPart then
						...
						local hasName=nm:find("bullet") or nm:find("proj") or nm:find("pellet") or nm:find("shot") or nm:find("ball") or nm:find("rocket") or nm:find("grenade") or nm:find("ammo") or nm:find("fire")
						if isSmall or hasName then pcall(function() obj.CanCollide=false end) end
					end
				end
			end
		end)
...
local function createParticle()
	...
	return {frame=p, x=math.random(2,WIN_W_P-4), y=math.random(2,WIN_H_P-4), vx=(math.random()-0.5)*22, vy=(math.random()-0.5)*22, size=size}
end
...
local function startParticles()
	...
	particleConn = RunService.RenderStepped:Connect(function(dt)
		...
		for _, d in ipairs(particles) do
			...
			if d.x < 2 then d.x = 2; d.vx = math.abs(d.vx)
			elseif d.x > WIN_W_P - d.size - 2 then d.x = WIN_W_P - d.size - 2; d.vx = -math.abs(d.vx) end
			if d.y < 2 then d.y = 2; d.vy = math.abs(d.vy)
			elseif d.y > WIN_H_P - d.size - 2 then d.y = WIN_H_P - d.size - 2; d.vy = -math.abs(d.vy) end
			pcall(function() d.frame.Position = UDim2.new(0, winX + d.x, 0, winY + d.y); d.frame.BackgroundColor3 = col end)
		end
	end)
end
...
task.defer(function()
	for i = 1, PARTICLE_COUNT do particles[i] = createParticle() end
	local winX = Main.AbsolutePosition.X; local winY = Main.AbsolutePosition.Y
	for _, d in ipairs(particles) do d.frame.Position = UDim2.new(0, winX + d.x, 0, winY + d.y) end
	startParticles()
end)
...
-- Sidebar
local SIDEBAR_W=155
local Sidebar=Instance.new("Frame",Main); Sidebar.Name="Sidebar"; Sidebar.Size=UDim2.new(0,SIDEBAR_W,1,-46); Sidebar.Position=UDim2.new(0,0,0,46); Sidebar.BackgroundColor3=C.sidebar; Sidebar.BorderSizePixel=0; Sidebar.ZIndex=3; Sidebar.ClipsDescendants=false
local SideDiv=Instance.new("Frame",Main); SideDiv.Size=UDim2.new(0,1,1,-46); SideDiv.Position=UDim2.new(0,SIDEBAR_W,0,46); SideDiv.BackgroundColor3=C.border; SideDiv.BorderSizePixel=0; SideDiv.ZIndex=4
local SideInner=Instance.new("Frame",Sidebar); SideInner.Name="SideInner"; SideInner.Size=UDim2.new(1,0,1,0); SideInner.Position=UDim2.new(0,0,0,0); SideInner.BackgroundTransparency=1; SideInner.BorderSizePixel=0; SideInner.ZIndex=3
local SideLayout=Instance.new("UIListLayout",SideInner); SideLayout.SortOrder=Enum.SortOrder.LayoutOrder; SideLayout.Padding=UDim.new(0,2)
local SidePad=Instance.new("UIPadding",SideInner); SidePad.PaddingTop=UDim.new(0,8); SidePad.PaddingLeft=UDim.new(0,7); SidePad.PaddingRight=UDim.new(0,7)
...
local function makeToggle(parent,label,desc,order,callback)
	...
	local function refresh()
		if state then
			TweenService:Create(Row,TW,{BackgroundColor3=C.cardActive}):Play(); TweenService:Create(Lbl,TW,{TextColor3=C.textPrim}):Play()
			TweenService:Create(Ind,TW,{BackgroundColor3=C.green}):Play(); TweenService:Create(Switch,TW,{BackgroundColor3=C.greenDim}):Play()
			TweenService:Create(Knob,TW,{Position=UDim2.new(1,-19,0.5,-8),BackgroundColor3=C.green}):Play(); SwStroke.Color=C.greenDim
		else
			TweenService:Create(Row,TW,{BackgroundColor3=C.card}):Play(); TweenService:Create(Lbl,TW,{TextColor3=C.textSec}):Play()
			TweenService:Create(Ind,TW,{BackgroundColor3=C.textDim}):Play(); TweenService:Create(Switch,TW,{BackgroundColor3=Color3.fromRGB(36,30,52)}):Play()
			TweenService:Create(Knob,TW,{Position=UDim2.new(0,3,0.5,-8),BackgroundColor3=C.textDim}):Play(); SwStroke.Color=C.border
		end
	end
	...
	return function(s) if s ~= state then state = s; refresh() end end
end
...
local function makeDropdown(parent, label, options, default, order, callback)
	...
	local Wrap = Instance.new("Frame", parent); Wrap.Name = "Dropdown_"..label; Wrap.Size = UDim2.new(1, 0, 0, 46); Wrap.BackgroundColor3 = C.card; Wrap.BorderSizePixel = 0; Wrap.LayoutOrder = order; Wrap.ZIndex = 5; Wrap.ClipsDescendants = false
	Instance.new("UICorner", Wrap).CornerRadius = UDim.new(0, 7)
	local WStroke = Instance.new("UIStroke", Wrap); WStroke.Color = C.border; WStroke.Thickness = 1
	local Ind = Instance.new("Frame", Wrap); Ind.Size = UDim2.new(0, 3, 0, 18); Ind.Position = UDim2.new(0, 0, 0.5, -9); Ind.BackgroundColor3 = C.accent; Ind.BorderSizePixel = 0; Instance.new("UICorner", Ind).CornerRadius = UDim.new(1, 0)
	local FieldLbl = Instance.new("TextLabel", Wrap); FieldLbl.Size = UDim2.new(1, -16, 0, 12); FieldLbl.Position = UDim2.new(0, 13, 0, 5); FieldLbl.BackgroundTransparency = 1; FieldLbl.Text = string.upper(label); FieldLbl.TextColor3 = C.textDim; FieldLbl.Font = Enum.Font.GothamBold; FieldLbl.TextSize = 9; FieldLbl.TextXAlignment = Enum.TextXAlignment.Left; FieldLbl.ZIndex = 6
	local ValLbl = Instance.new("TextLabel", Wrap); ValLbl.Size = UDim2.new(1, -40, 0, 18); ValLbl.Position = UDim2.new(0, 13, 0, 20); ValLbl.BackgroundTransparency = 1; ValLbl.Text = options[selected]; ValLbl.TextColor3 = C.textPrim; ValLbl.Font = Enum.Font.GothamBold; ValLbl.TextSize = 13; ValLbl.TextXAlignment = Enum.TextXAlignment.Left; ValLbl.ZIndex = 6
	local Arrow = Instance.new("TextLabel", Wrap); Arrow.Size = UDim2.new(0, 20, 0, 20); Arrow.Position = UDim2.new(1, -28, 0.5, -10); Arrow.BackgroundTransparency = 1; Arrow.Text = "v"; Arrow.TextColor3 = C.accentLight; Arrow.Font = Enum.Font.GothamBold; Arrow.TextSize = 12; Arrow.ZIndex = 6
	local OpenBtn = Instance.new("TextButton", Wrap); OpenBtn.Size = UDim2.new(1, 0, 1, 0); OpenBtn.BackgroundTransparency = 1; OpenBtn.Text = ""; OpenBtn.ZIndex = 7
	local DropList = Instance.new("Frame", Wrap); DropList.Size = UDim2.new(1, 0, 0, 0); DropList.Position = UDim2.new(0, 0, 1, 6); DropList.BackgroundColor3 = C.dropBG; DropList.BorderSizePixel = 0; DropList.ZIndex = 80; DropList.Visible = false; DropList.ClipsDescendants = true
	Instance.new("UICorner", DropList).CornerRadius = UDim.new(0, 8)
	local DStroke = Instance.new("UIStroke", DropList); DStroke.Color = C.accentLight; DStroke.Thickness = 1; DStroke.Transparency = 0.6
	local DPad = Instance.new("UIPadding", DropList); DPad.PaddingTop = UDim.new(0, 4); DPad.PaddingBottom = UDim.new(0, 4); DPad.PaddingLeft = UDim.new(0, 5); DPad.PaddingRight = UDim.new(0, 5)
	local DLayout = Instance.new("UIListLayout", DropList); DLayout.SortOrder = Enum.SortOrder.LayoutOrder; DLayout.Padding = UDim.new(0, 2)
	local itemRefs = {}
	for i, opt in ipairs(options) do
		local Item = Instance.new("TextButton", DropList); Item.Size = UDim2.new(1, 0, 0, LIST_ITEM_H); Item.BackgroundColor3 = C.dropItemHov; Item.BackgroundTransparency = (i == selected) and 0.55 or 1; Item.BorderSizePixel = 0; Item.Text = ""; Item.LayoutOrder = i; Item.ZIndex = 82; Instance.new("UICorner", Item).CornerRadius = UDim.new(0, 6)
		local Dot = Instance.new("Frame", Item); Dot.Size = UDim2.new(0, 5, 0, 5); Dot.Position = UDim2.new(0, 9, 0.5, -2.5); Dot.BackgroundColor3 = C.accentLight; Dot.BorderSizePixel = 0; Dot.ZIndex = 83; Dot.Visible = (i == selected); Instance.new("UICorner", Dot).CornerRadius = UDim.new(1, 0)
		local OptLbl = Instance.new("TextLabel", Item); OptLbl.Size = UDim2.new(1, -24, 1, 0); OptLbl.Position = UDim2.new(0, 20, 0, 0); OptLbl.BackgroundTransparency = 1; OptLbl.Text = opt; OptLbl.TextColor3 = (i == selected) and C.accentLight or C.textSec; OptLbl.Font = (i == selected) and Enum.Font.GothamBold or Enum.Font.Gotham; OptLbl.TextSize = 12; OptLbl.TextXAlignment = Enum.TextXAlignment.Left; OptLbl.ZIndex = 83
		Item.MouseEnter:Connect(function() if i ~= selected then TweenService:Create(Item,TweenInfo.new(0.08),{BackgroundTransparency=0.7}):Play() end end)
		Item.MouseLeave:Connect(function() if i ~= selected then TweenService:Create(Item,TweenInfo.new(0.08),{BackgroundTransparency=1}):Play() end end)
		Item.MouseButton1Click:Connect(function()
			for _, ref in ipairs(itemRefs) do ref.dot.Visible = false; ref.lbl.TextColor3 = C.textSec; ref.lbl.Font = Enum.Font.Gotham; TweenService:Create(ref.btn,TweenInfo.new(0.08),{BackgroundTransparency=1}):Play() end
			Dot.Visible = true; OptLbl.TextColor3 = C.accentLight; OptLbl.Font = Enum.Font.GothamBold; TweenService:Create(Item,TweenInfo.new(0.08),{BackgroundTransparency=0.55}):Play()
			selected = i; ValLbl.Text = opt
			...
			TweenService:Create(Arrow,TweenInfo.new(0.12),{Rotation=0}):Play(); TweenService:Create(WStroke,TweenInfo.new(0.12),{Color=C.border}):Play()
			_openDropList = nil; callback(opt)
		end)
		table.insert(itemRefs, {btn=Item, dot=Dot, lbl=OptLbl})
	end
	local isOpen = false
	OpenBtn.MouseButton1Click:Connect(function()
		if _openDropList and _openDropList ~= DropList then _openDropList.Visible = false; _openDropList.Size = UDim2.new(1,0,0,0) end
		isOpen = not isOpen
		if isOpen then
			_openDropList = DropList; DropList.Size = UDim2.new(1,0,0,0); DropList.Visible = true
			...
			TweenService:Create(Arrow,TweenInfo.new(0.15),{Rotation=180}):Play(); TweenService:Create(WStroke,TweenInfo.new(0.15),{Color=C.accentLight}):Play()
		else
			_openDropList = nil
			TweenService:Create(DropList,TweenInfo.new(0.14,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Size=UDim2.new(1,0,0,0)}):Play()
			task.delay(0.15,function() DropList.Visible=false end)
...
-- Populando paginas (identico ao original)
...
local function buildMisc()
	...
	toggleSetters["MISC.Thirdperson"] = makeToggle(p,"Thirdperson","Visao em terceira pessoa",8,function(s)
		...
		if s then
			tpConn=RunService.RenderStepped:Connect(function()
				...
				local char=LocalPlayer.Character; local hrp=char and char:FindFirstChild("HumanoidRootPart")
				if hrp then Camera.CFrame=CFrame.new(hrp.Position + Vector3.new(0,4,0) + Camera.CFrame.LookVector*-MISC.ThirdpersonDist, hrp.Position + Vector3.new(0,2,0)); Camera.CameraType=Enum.CameraType.Scriptable end
			end)
		else
	...
	toggleSetters["MISC.BulletTracer"] = makeToggle(p,"Bullet Tracer","Mostra o rastro das suas balas",24,function(s)
		MISC.BulletTracer=s
		for _,c in pairs(bulletConns) do pcall(function() c:Disconnect() end) end; bulletConns={}
		...
		local function hookTool(tool)
			...
			local fireConn=tool.ChildAdded:Connect(function(obj)
				...
				pcall(function()
					local line=Drawing.new("Line"); line.Thickness=1.5; line.Color=C.accentLight; line.Transparency=1
					local startPos=obj.Position
					local moved=obj:GetPropertyChangedSignal("Position"):Connect(function()
						local v1,_=Camera:WorldToViewportPoint(startPos); local v2,_=Camera:WorldToViewportPoint(obj.Position)
	...
	local function updateCrosshair()
		if not MISC.Crosshair then for _,l in pairs(crosshairLines) do pcall(function() l:Remove() end) end; crosshairLines={}; return end
		local vp=Camera.ViewportSize; local cx,cy=vp.X/2,vp.Y/2; local sz=MISC.CrosshairSize or 10; local gap=MISC.CrosshairGap or 4; local col=C.accentLight
		if #crosshairLines==0 then for i=1,4 do local l=Drawing.new("Line"); l.Thickness=1.5; l.Color=col; l.Transparency=1; l.Visible=true; crosshairLines[i]=l end end
	...
	makeSlider(p,"Tamanho Radar",100,240,160,"px",40,function(v) RADAR_SIZE=v; RADAR_RADIUS=v/2-6; RadarGui.Size=UDim2.new(0,v,0,v) end)
end
buildMisc()
...
local function buildConfig()
	...
	toggleSetters["CONFIG.Particles"]     = makeToggle(p,"Particulas","Estrelinhas animadas no fundo",4,function(s)
		CONFIG.Particles=s
		if s then setParticlesVisible(true); startParticles() else stopParticles() end
	end)
	toggleSetters["CONFIG.Watermark"]     = makeToggle(p,"Watermark","FPS, ping e versao no canto da tela",5,function(s)
		CONFIG.Watermark=s; if WatermarkFrame then WatermarkFrame.Visible=s end
	end)
	makeSpacer(p,6,6)
	makeSlider(p,"Opacidade",20,100,100,"%",7,function(v) CONFIG.Opacity=v/100; Main.BackgroundTransparency=1-(v/100) end)
	...
	local function applyTheme(name)
		...
		local oldAccent=C.accent; local oldAccentLight=C.accentLight; local oldAccentGlow=C.accentGlow
		local h,s,v=Color3.toHSV(t.neon); local newGlow=Color3.fromHSV(h,s,math.max(0,v-0.25))
		C.accent=t.border; C.accentLight=t.neonBright; C.accentGlow=newGlow
		...
		MainStroke.Color=C.accent; HDivider.BackgroundColor3=C.accent; LogoDot.BackgroundColor3=C.accentLight
		VerBadge.BackgroundColor3=C.accentGlow; VerBadge.TextColor3=C.accentLight; fovCircle.Color=C.accentLight
		for _,d in ipairs(particles) do pcall(function() d.frame.BackgroundColor3=C.accentLight end) end
		if WatermarkFrame then WatermarkStroke.Color=C.accent; WatermarkDot.BackgroundColor3=C.accentLight end
	end
	...
	local PROFILES={"Rage","Legit","Visual Only"}; local profileData={}
	local function captureConfig()
		return {
			ESP={Enabled=ESP.Enabled,RGB=ESP.RGB,Box=ESP.Box,Skeleton=ESP.Skeleton,Names=ESP.Names,Distance=ESP.Distance,HealthBar=ESP.HealthBar,Tracers=ESP.Tracers,Radar=ESP.Radar,VisionCone=ESP.VisionCone},
			AIMBOT={Enabled=AIMBOT.Enabled,Silent=AIMBOT.Silent,Prediction=AIMBOT.Prediction,FOVCircle=AIMBOT.FOVCircle,TeamCheck=AIMBOT.TeamCheck,VisCheck=AIMBOT.VisCheck,AutoShoot=AIMBOT.AutoShoot,FOVRadius=AIMBOT.FOVRadius,Smoothness=AIMBOT.Smoothness,MaxDist=AIMBOT.MaxDist,TargetPart=AIMBOT.TargetPart},
			MISC={BigHead=MISC.BigHead,BigHeadScale=MISC.BigHeadScale,SpinBot=MISC.SpinBot,Noclip=MISC.Noclip},
		}
	end
	...
	for i,name in ipairs(PROFILES) do
		...
		loadBtn.MouseButton1Click:Connect(function()
			...
			if d.MISC then if d.MISC.BigHead~=nil then onBigHeadToggle(d.MISC.BigHead) end end
	...
	KEYBINDS.AimbotKey=KEYBINDS.AimbotKey or nil; KEYBINDS.SilentKey=KEYBINDS.SilentKey or nil; KEYBINDS.NoclipKey=KEYBINDS.NoclipKey or nil
...
UserInputService.InputBegan:Connect(function(input,gpe)
	...
	if KEYBINDS.ESPKey and input.KeyCode==KEYBINDS.ESPKey then
		...
		onESPToggle(ESP.Enabled); showToast((ESP.Enabled and "[ON]  " or "[OFF] ").."ESP",ESP.Enabled)
	end
	if KEYBINDS.BigHeadKey and input.KeyCode==KEYBINDS.BigHeadKey then
		...
		onBigHeadToggle(MISC.BigHead); showToast((MISC.BigHead and "[ON]  " or "[OFF] ").."Big Head",MISC.BigHead)
	end
	...
	if KEYBINDS.NoclipKey and input.KeyCode==KEYBINDS.NoclipKey then
		...
		onNoclipToggle(MISC.Noclip); showToast((MISC.Noclip and "[ON]  " or "[OFF] ").."Noclip",MISC.Noclip)
	end
end)
